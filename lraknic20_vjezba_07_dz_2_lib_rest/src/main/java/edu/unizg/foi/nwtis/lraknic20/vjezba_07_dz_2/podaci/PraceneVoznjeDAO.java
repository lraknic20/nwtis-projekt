...
public class PraceneVoznjeDAO {
  ...
  public List<Vozilo> dohvatiPraceneVoznje(long odVremena, long doVremena) {
  ...
  public List<Vozilo> dohvatiPracenoVozilo(int id) {
    String upit = "SELECT * FROM pracenevoznje WHERE id = ?";

    List<Vozilo> pracenoVozilo = new ArrayList<>();
    ...
    try (PreparedStatement s = this.vezaBP.prepareStatement(upit)) {
      ...
      while (rs.next()) {
        var vozilo = kreirajObjektVozilo(rs);
        pracenoVozilo.add(vozilo);
      }
    } catch (SQLException ex) {
      Logger.getLogger(PraceneVoznjeDAO.class.getName()).log(Level.SEVERE, null, ex);
    }
    return pracenoVozilo;
  }

  public List<Vozilo> dohvatiPracenoVoziloOdDo(int id, long odVremena, long doVremena) {
  ...
  public boolean dodajVozilo(Vozilo novoVozilo) {
    String upit = "INSERT INTO pracenevoznje (id, broj, vrijeme, brzina, snaga, struja, visina,"
